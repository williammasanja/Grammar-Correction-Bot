...
text = 'This is a test file the test file will containz 4 sentenceh This is the third sentence i like carz'

#make the 6 lists to 3-4 if possible
wrong_words = []
wrong_word = []
seperate_text = []
new_text = []
...
def word_lookup(list):
    ...
    for word in list:
        ...
        elif word[len(word)-1] == "s" and word[len(word)-1] in txt_file: 
           #print(word)
           word_count += 1
    ...
    print("{} out found in the wordbase. {} word(s) not found".format(word_count, len(wrong_words)))
    print(wrong_words)
    ...
    for i in range(len(wrong_words)):
        ...
        if first_char in Alphabet_string:
            ...
            # make lines 71 to 76 into a seperate file to add functions in the other code(main.py) if needed last minute
            ...
            for x in range(len(index_file)):
                if len(index_file[x]) == len(wrong_words[i]):
                    words_that_are_testing[index_file[x]] = 0
                elif len(index_file[x])-1 == len(wrong_words[i]) or  len(index_file[x])+1 == len(wrong_words[i]):
                    words_that_are_testing[index_file[x]] = 0
            ...
            # First Step of Spelling correction. lines 91-96 looks at both the wrong_word_chars and each word in the Dictionary: words_that_are_testing to see if they share similar characters and counts each one.
